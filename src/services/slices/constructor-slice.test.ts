...
describe('constructor-slice reducer', () => {
  const empty = { bun: null, ingredients: [] as any[] };
  ...
  it('добавляет булку через setBun (id генерируется prepare)', () => {
    const state1 = burgerConstructorReducer(empty, setBun(bun));
  ...
  it('добавляет ингредиент через addIngredient (id генерируется prepare)', () => {
    const state1 = burgerConstructorReducer(empty, addIngredient(filling1));
  ...
  it('меняет порядок ингредиентов (moveIngredient)', () => {
    const s1 = burgerConstructorReducer(empty, addIngredient(filling1));
  ...
  it('очищает конструктор', () => {
    const s1 = burgerConstructorReducer(empty, setBun(bun));
